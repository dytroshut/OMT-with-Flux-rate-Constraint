%% Generalized multimarginal problem (weighted cost): Guaasian Case
...
C = epsilon.*(X+0.4).^2./T1 + (0.8).^2./(T2-T1) + epsilon.*(Y-0.4).^2./(T-T2);
...
[Iqt,Jqt,~] = find(qt2Coupling>0.001);
...
[Itt,Jtt,~] = find(tollCoupling>0.001);
