...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
p = 5.*Gaussian(x, -.7,sigma3) + 1.5.*Gaussian(x, -.6,sigma0) + 2.5.*Gaussian(x, -.5,sigma2);
% % Marginal q
q = 0.2.*Gaussian(y, .2,sigma4)+ 3.*Gaussian(y, .3,sigma2)+ 1.5*Gaussian(y, .4,sigma2)+ 1*Gaussian(y, .7,sigma2)+ 5.*Gaussian(y,.5,sigma3);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%      pt_assignement = plot3(x(I(i)),y(J(i)),t(K(i)),'o','Color','k','MarkerSize',...
%      1000*Pi(I(i),J(i),K(i)),'MarkerFaceColor','k');
     pt_assignement = plot3(x(I(i)),y(J(i)),t(K(i)),'o','MarkerSize',...
     1000*Pi(I(i),J(i),K(i)),'MarkerFaceColor','k');
 end
