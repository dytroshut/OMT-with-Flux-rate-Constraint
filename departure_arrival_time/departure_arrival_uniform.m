...
x = (0.01:n1-1+0.01)'/n1-1; % x \in [-1,0]
y = (1-0.0101:n2-0.0101)'/n2;     % y \in [ 0,1]
...
% p((n1*1/5-5):(n1*1/5+5)) = 1;  % 'Dirac 2'

q = zeros(size(y));
q(( n2/2 - 6):( n2/2 + 6)) = 2;      % 'Dirac 1'
% q((2*n2/3-3):(2*n2/3+3)) = 3;      % 'Dirac 2'
...
t0 = T*(1-0.001:nt0-0.001)'/nt0;
t1 = T*(1-0.001:nt1-0.001)'/nt1;
...
list2 = find(Dt==0);
YY1 = [t0(list1(1)) t0(list1(1)) t0(list1(end)) t0(list2(1))];
...
[I,J] = find(qt1Coupling>0.005);
 for i=1:length(I)
     ...
     200*qt1Coupling(I(i),J(i)),'MarkerFaceColor','k');
 end
hold off
axis tight;
...
[I,J] = find(pt0Coupling>0.005);
 for i=1:length(I)
     pt_assignement = plot(x(I(i)),t0(J(i)),'o','Color','k','MarkerSize',...
     150*pt0Coupling(I(i),J(i)),'MarkerFaceColor','k');
 end
...
[I,J] = find(qt1Coupling>0.005);
 for i=1:length(I)
     pt_assignement = plot(y(I(i)),t1(J(i)),'o','Color','k','MarkerSize',...
     150*qt1Coupling(I(i),J(i)),'MarkerFaceColor','k');
 end
hold off
