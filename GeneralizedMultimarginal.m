...
C = (X+0.4).^2./T1 + (0.6).^2./(T2-T1) + (Y-0.2).^2./(T-T2);
...
for iqt=1:length(Iqt)
    qt_assignement = plot([yq(Iqt(iqt),1),tx2(Jqt(iqt))],[yq(Iqt(iqt),2),ty2(Jqt(iqt))],'LineWidth',3,'Color',[0.6 0.6 .6]);
end

% tolls Coupling (order fixed):
[Itt,Jtt,~] = find(tollCoupling);
for iqt=1:length(Itt)
    qt_assignement = plot([tx1(Itt(iqt)),tx2(Jtt(iqt))],[ty1(Itt(iqt)),ty2(Jtt(iqt))],'LineWidth',3,'Color',[0.6 0.6 .6]);
end
